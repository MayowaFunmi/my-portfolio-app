    ...
    if request.method == 'POST':
        ...
        try:
            if form.is_valid():
                ...
                name_str = name.lower()
                for i in range(0, len(name_str), 1):
                    if name_str[i] == ' ':
                        name_str = name_str.replace(name_str[i], '-')
                print(name_str)
            ...
            print(form.errors)
            messages.error(request, 'Your Project Failed To Be Posted')
            return HttpResponseRedirect('/users/add_project/')
